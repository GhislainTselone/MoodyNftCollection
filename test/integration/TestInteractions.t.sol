...
import {StartMinting, GetDisplayedImageUri} from "../../script/Interactions.s.sol";

contract TestInteractions is Test {
    MoodyNft private s_moodyNft;
    Helper private s_helper;
    StartMinting private s_startMinting;
    GetDisplayedImageUri private s_imageUri;
    ...
    function setUp() external {
        s_startMinting = new StartMinting();
        s_imageUri = new GetDisplayedImageUri();
        DeployMoodyNft deploy = new DeployMoodyNft();
        (s_helper, s_moodyNft) = deploy.run();
    }

    function testStartMintingAndGetDisplayedImageUri() external {
        s_startMinting.startMinting(HAPPY_MOOD, SAD_MOOD, address(s_moodyNft));
        string memory displayImageUri = s_imageUri.displayedImageUri(
            0,
            address(s_moodyNft)
        );
